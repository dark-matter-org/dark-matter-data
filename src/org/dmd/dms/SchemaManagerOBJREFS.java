...
public class SchemaManagerOBJREFS implements DmcNameResolverIF {
...
//    StringName nameKey;
     ...
     */
    public SchemaManagerOBJREFS() throws ResultException, DmcValueException {
    	init();
    }
     ...
     * @throws DmcValueException
     */
    public SchemaManagerOBJREFS(SchemaDefinition sd) throws ResultException, DmcValueException {
    	init();
    	manageSchema(sd);
    }
...
//        cd.setIsNamedBy(MetaSchemaAG._ruleName);
    ...
    void addClass(ClassDefinition cd) throws ResultException, DmcValueException {
        ...
        // For the meta schema, the internally generated type for references have already been
        // loaded, so we don't add them to the all definitions map
        if (cd.getDefinedIn() != MetaSchemaAG._metaSchema){
...
//    	}
    	if ((td.getDefinedIn() == MetaSchemaAG._metaSchema) &&(td.getOriginalClass() != null)){
    		internalTypeDefs.put(td.getName(), td);
    	}
    	else{
...
//		}
     ...
     */
    public void diff(SchemaManagerOBJREFS predecessor, StringBuffer sb){
     ...
     */
    int newSchemas(SchemaManagerOBJREFS predecessor, StringBuffer sb){
     ...
     */
    int deletedSchemas(SchemaManagerOBJREFS predecessor, StringBuffer sb){
     ...
     *
     */
    int newClasses(SchemaManagerOBJREFS predecessor, StringBuffer sb){
        Iterator<SchemaDefinition>        it  = schemaDefs.values().iterator();
        int             rc  =   0;
     ...
     *
     */
    int newAttributes(SchemaManagerOBJREFS predecessor, StringBuffer sb){
        Iterator<SchemaDefinition>        it  = schemaDefs.values().iterator();
        int             rc  =   0;
...
//                int changes = curr.attributeChanges(pred,attrDiff,this);
     ...
     *
     */
    int newTypes(SchemaManagerOBJREFS predecessor, StringBuffer sb){
        Iterator<SchemaDefinition>        it  = schemaDefs.values().iterator();
        int             rc  =   0;
