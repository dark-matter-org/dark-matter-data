...
import org.dmd.dms.server.generated.dmw.DSDefinitionModuleIterableDMW;
import org.dmd.dms.server.util.DmcDefinitionSet;
...
import org.dmd.dms.shared.types.RuleName;
...
//    public HashMap<DefinitionName,ClassDefinition>     			classDefs;
     ...
     * @throws DmcNameClashException 
     */
    public SchemaManager(SchemaDefinition sd) throws DMFeedbackSet {
    	init();
    	manageSchema(sd);
    }
    
    void init() throws DMFeedbackSet {
...
//    		for(ClassDefinition cd: classDefs.values()){
    ...
    public DmwWrapper wrapIt(DmcObject dmo) throws DmcNameClashException, DMFeedbackSet {
		...
		try {
			...
			e.add(new DMFeedback("Cannot create DmwWrapper for class: " + dmo.getConstructionClassName()));
    		throw(e);
		}
    	rc.setDmcObject(dmo);
    	return(rc);
	 ...
	 * @throws DmcValueException  
	 */
	public void definitionPreAdd(DMUncheckedObject def) throws DMFeedbackSet {
    	for(SchemaExtensionIF currext : extensions.values()){
     ...
     * @throws DmcNameClashException 
     */
	public void schemaPreAdd(DMUncheckedObject sd) throws DMFeedbackSet, DmcNameClashException {
     	...
     	if (attr != null){
    		...
    		for(String ext: attr){
                try{
                	...
                	DMFeedback error = new DMFeedback("Couldn't load schemaExtension class: " + ext);
                ...
                if (extensions.get(className) == null){
                    ...
                    try{
                    	...
                    	DMFeedback error = new DMFeedback("Couldn't instantiate Java class: " + ext);
                    	error.addToMessage("This may be because the class doesn't have a constructor that takes no arguments.");
                    	error.addToMessage("Or it may be that the class doesn't implement the SchemaExtensionIF interface.");
                        throw(new DMFeedbackSet(error));
                    }
     ...
     * @throws DmcNameClashException 
     */
	private void loadGeneratedSchema(SchemaDefinition sd) throws DMFeedbackSet, DmcNameClashException {
    	...
    	for(String schemaName : sd.dependsOnSchemaClasses.keySet()){
			...
			if (depSchema == null){
                ...
                try{
                	...
                	DMFeedback error = new DMFeedback("Couldn't load generated schema class: " + schemaClassName);
                	error.addToMessage(e.getMessage());
                	error.addToMessage(DebugInfo.extractTheStack(e));
                    throw(new DMFeedbackSet(error));
                }
                ...
                try{
                	...
                	DMFeedback error = new DMFeedback("Couldn't instantiate Java class: " + schemaClassName);
                	error.addToMessage("This may be because the class doesn't have a constructor that takes no arguments.");
                	error.addToMessage("Or it may be that the class isn't derived from SchemaDefinition.");
                    throw(new DMFeedbackSet(error));
                }
     ...
     * @throws DmcNameClashException 
     */
    public void manageSchema(SchemaDefinition sd) throws DMFeedbackSet, DmcNameClashException {
    ...
    public void manageSchemaInternal(SchemaDefinition sd, boolean checkIDs) throws DMFeedbackSet, DmcNameClashException {
     ...
     * @throws DmcNameClashException
     */
    public void manageSchemaInternal(SchemaDefinition sd) throws DMFeedbackSet, DmcNameClashException {
     ...
     * @throws DmcValueException 
     */
    public ClassDefinition isClass(String name) throws DmcNameClashException, DMFeedbackSet {
...
//        return((ClassDefinition)classDefs.get(getDefName(name)));
     ...
     * @throws DmcValueException 
     */
    void addSchema(SchemaDefinition sd) throws DMFeedbackSet {
        currentSchema = sd;

        if (checkAndAdd(sd.getObjectName(),sd,schemaDefs) == false){
        	DMFeedback error = new DMFeedback(clashMsg(sd.getObjectName(),sd,schemaDefs,"schema names"));
            currentSchema = null;
        	throw(new DMFeedbackSet(error));
        }

        if (checkAndAddDOT(sd.getDotName(),sd,globallyUniqueMAP,null) == false){
        	DMFeedback error = new DMFeedback(clashMsgDOT(sd.getObjectName(),sd,globallyUniqueMAP,"definition names"));
            currentSchema = null;
        	throw(new DMFeedbackSet(error));
        }
     ...
     * @throws DmcValueException
     */
    void addDefinitionModule(DSDefinitionModule ddm, boolean managingSchema) throws DMFeedbackSet {

        if (checkAndAdd(ddm.getObjectName(),ddm,definitionModuleDefs) == false){
        	DMFeedback error = new DMFeedback(clashMsg(ddm.getObjectName(),ddm,definitionModuleDefs,"DSDefinitionModule names"));
            currentSchema = null;
        	throw(new DMFeedbackSet(error));
        }
        
        if (checkAndAddDOT(ddm.getDotName(),ddm,globallyUniqueMAP,null) == false){
        	DMFeedback error = new DMFeedback(clashMsgDOT(ddm.getObjectName(),ddm,globallyUniqueMAP,"definition names"));
            throw(new DMFeedbackSet(error));
        }
        ...
        if (ddm.getDmdID() != null){
        	if (ddm.getDmdID() != 1){
            	DMFeedback error = new DMFeedback("The dmdID of a DSDefinitionModule must be 1", ddm.getFile(), ddm.getLineNumber());
        		throw(new DMFeedbackSet(error));
        	}
        }
        ...
        try {
			...
			DMFeedbackSet ex = new DMFeedbackSet("Unresolved references in DSDefinitionModule: " + ddm.getName());
			ex.add(e);
			throw(ex);
		}
...
//        cd.addMay(ddm.getModuleDependenceAttribute());
     ...
     * @throws DmcValueException  
     */
    void addDependenceAttributes(ClassDefinition cd, DSDefinitionModule module, TreeSet<String> modules) throws DMFeedbackSet {
     ...
     * @throws DmcValueExceptionSet 
     */
    void addComplexType(ComplexTypeDefinition ctd) throws DMFeedbackSet {
        if (checkAndAdd(ctd.getObjectName(),ctd,complexTypeDefs) == false){
        	throw(new DMFeedbackSet(clashMsg(ctd.getObjectName(),ctd,complexTypeDefs,"complex type names")));
        }
        
        if (checkAndAddDOT(ctd.getDotName(),ctd,globallyUniqueMAP,null) == false){
            currentSchema = null;
        	throw(new DMFeedbackSet(clashMsgDOT(ctd.getObjectName(),ctd,globallyUniqueMAP,"definition names")));
        }        
     ...
     * @throws DMFeedbackSet 
     */
    void addExtendedReferenceType(ExtendedReferenceTypeDefinition ertd) throws DMFeedbackSet {
        if (checkAndAdd(ertd.getObjectName(),ertd,extendedReferenceTypeDefs) == false){
        	throw(new DMFeedbackSet(clashMsg(ertd.getObjectName(),ertd,extendedReferenceTypeDefs,"extended reference type names")));
        }
        
        if (checkAndAddDOT(ertd.getDotName(),ertd,globallyUniqueMAP,null) == false){
            currentSchema = null;
        	throw(new DMFeedbackSet(clashMsgDOT(ertd.getObjectName(),ertd,globallyUniqueMAP,"definition names")));
        }        
     ...
     * @throws DMFeedbackSet 
     */
    void addSlice(SliceDefinition sd) throws DMFeedbackSet {
        if (checkAndAdd(sd.getObjectName(),sd,sliceDefs) == false){
        	throw(new DMFeedbackSet(clashMsg(sd.getObjectName(),sd,sliceDefs,"slice names")));
        }
        
        if (checkAndAddDOT(sd.getDotName(),sd,globallyUniqueMAP,null) == false){
        	throw(new DMFeedbackSet(clashMsgDOT(sd.getObjectName(),sd,globallyUniqueMAP,"definition names")));
        }
    }
     ...
     * @throws DMFeedbackSet 
     */
    void addRuleCategory(RuleCategory rc) throws DMFeedbackSet {
        if (checkAndAdd(rc.getObjectName(),rc,ruleCategoryDefs) == false){
        	throw(new DMFeedbackSet(clashMsg(rc.getObjectName(),rc,ruleCategoryDefs,"rule categories")));
        }
        
        if (checkAndAddDOT(rc.getDotName(),rc,globallyUniqueMAP,null) == false){
        	throw(new DMFeedbackSet(clashMsgDOT(rc.getObjectName(),rc,globallyUniqueMAP,"definition names")));
        }
        ...
        if (performIDChecks){
	        ...
	        if (current >= range){
	        	throw(new DMFeedbackSet("Number of rule categories exceeds schema ID range: " + rc.getName()));
	        }
        ...
        if (ruleCategoriesByID.get(rc.getRuleCategoryID()) != null){
        	throw(new DMFeedbackSet(clashingIDsMsg(rc.getRuleCategoryID(),rc,ruleCategoriesByID,"ruleCategoryID")));
        }
    }
     ...
     * @throws DmcNameClashException 
     */
    void addRuleDefinition(RuleDefinition rd) throws DMFeedbackSet, DmcNameClashException {
        ...
        try {
			...
			DMFeedbackSet ex = new DMFeedbackSet("Unresolved references in RuleDefinition: " + rd.getName());
			ex.add(e);
			throw(ex);
		}
        ...
        if (checkAndAdd(rd.getObjectName(),rd,ruleDefs) == false){
        	throw(new DMFeedbackSet(clashMsg(rd.getObjectName(),rd,ruleDefs,"rule definitions")));
        }
        ...
        if (checkAndAddDOT(rd.getDotName(),rd,globallyUniqueMAP,null) == false){
        	throw(new DMFeedbackSet(clashMsgDOT(rd.getObjectName(),rd,globallyUniqueMAP,"definition names")));
        }
        ...
        if (performIDChecks){
	        ...
	        if (current >= range){
	        	throw(new DMFeedbackSet("Number of rules exceeds schema ID range: " + rd.getName()));
	        }
        ...
        if (classesByID.get(rd.getDmdID()) != null){
        	throw(new DMFeedbackSet(clashingIDsMsg(rd.getDmdID(),rd,classesByID,"dmdID")));
        }
     ...
     * @throws DMFeedbackSet 
     */
    void addClass(ClassDefinition cd) throws DMFeedbackSet {
        ...
        if (checkAndAddDOT(cd.getDotName(),cd,globallyUniqueMAP,refName) == false){
        	throw(new DMFeedbackSet(clashMsgDOT(cd.getObjectName(),cd,globallyUniqueMAP,"definition names")));
        }

        ///////////////////////////////////////////////////////////////////////
        
        // TODO: this needs to be a rule
        if (cd.getDmdID() == null){	
        	throw(new DMFeedbackSet("Missing dmdID for class: " + cd.getName(), cd.getFile(), cd.getLineNumber()));
        }

        // TODO: this needs to be a rule
        if (cd.getDefinedIn() == null){
        	throw(new DMFeedbackSet("definedIn missing for class: " + cd.getName(), cd.getFile(), cd.getLineNumber()));
        }
        else{
        	if (performIDChecks){
	        	if ( (cd.getDefinedIn().getSchemaBaseID()  == null) || (cd.getDefinedIn().getSchemaIDRange() == null) ){
	            	throw(new DMFeedbackSet("schemaBaseID or schemaIDRange missing for schema: " + cd.getDefinedIn().getName()));
	        	}
        	}
        }
        ...
        if (performIDChecks){
	        ...
	        if (current >= range){
	        	throw(new DMFeedbackSet("Number of classes exceeds schema ID range: " + cd.getName()));
	        }
        ...
        if (classesByID.get(cd.getDmdID()) != null){
        	throw(new DMFeedbackSet(clashingIDsMsg(cd.getDmdID(),cd,classesByID,"dmdID")));
        }
        ...
        try {
			...
			DMFeedback error = new DMFeedback("Unresolved references in ClassDefinition: " + cd.getName());
			error.addToMessage(cd.toOIF());
...
//	        td.setNameAndTypeName(nameAndTypeName);
	        ...
	        if (cd.getIsNamedBy() != null){
	        	...
	        	if (cd.getIsNamedBy().getValueType() != ValueTypeEnum.SINGLE){
	        		DMFeedback error = new DMFeedback("The naming attribute: " + cd.getIsNamedBy().getName() + " for class: " + cd.getName() + " must be valueType SINGLE");
	        		error.setLocation(cd.getIsNamedBy().getFile(), cd.getIsNamedBy().getLineNumber());
	        		throw(new DMFeedbackSet(error));
	        	}
	        }
	        ...
	        if (checkAndAddDOT(td.getDotName(),td,globallyUniqueMAP,null) == false){
        		throw(new DMFeedbackSet(clashMsgDOT(td.getObjectName(),td,globallyUniqueMAP,"definition names")));
	        }
     ...
     * @throws DMFeedbackSet 
     */
    void addAttribute(AttributeDefinition ad) throws DMFeedbackSet {
    	
    	attributeDefinitions.add(ad);
    	
        if (checkAndAddDOT(ad.getDotName(),ad,globallyUniqueMAP,null) == false){
    		throw(new DMFeedbackSet(clashMsgDOT(ad.getObjectName(),ad,globallyUniqueMAP,"definition names")));
        }
        
        if (ad.getDmdID() == null){
    		throw(new DMFeedbackSet("Missing dmdID for attribute: " + ad.getName()));
        }
        
        if (ad.getDefinedIn() == null){
    		throw(new DMFeedbackSet("definedIn missing for attribute: " + ad.getName()));
        }
        else{
        	if (performIDChecks){
	        	if ( (ad.getDefinedIn().getSchemaBaseID()  == null) || (ad.getDefinedIn().getSchemaIDRange() == null) ){
	        		throw(new DMFeedbackSet("schemaBaseID or schemaIDRange missing for schema: " + ad.getDefinedIn().getName()));
	        	}
        	}
        ...
        if (performIDChecks){
	        ...
	        if (current >= range){
        		throw(new DMFeedbackSet("Number of attributes exceeds schema ID range: " + ad.getName()));
	        }
    	...
    	if (attrByID.get(ad.getDmdID()) != null){
    		throw(new DMFeedbackSet(clashingIDsMsg(ad.getDmdID(),ad,attrByID,"dmdID")));
        }
        ...
        if (ad.getAbbrev() != null){
            ...
            if (checkAndAddDOT(dotAbbrevName,ad,globallyUniqueMAP,null) == false){
        		...
        		throw(new DMFeedbackSet(clashMsgDOT(errName,ad,globallyUniqueMAP,"definition names")));
            }
     ...
     * @throws DMFeedbackSet 
     */
    void addType(TypeDefinition td) throws DMFeedbackSet {

        if (checkAndAdd(td.getObjectName(),td,typeDefs) == false){
    		throw(new DMFeedbackSet(clashMsg(td.getObjectName(),td,typeDefs,"type names")));
        }
     ...
     * @throws DmcNameClashException
     */
    public void addDefinition(DmsDefinition def, DmcNameClashResolverIF clashResolver) throws DMFeedbackSet, DmcNameClashException {
     ...
     * @throws DmcNameClashException 
     */
    void addDefinition(DmsDefinition def) throws DMFeedbackSet, DmcNameClashException {
    	...
    	// Note: test for extended ref before complex type because it is derived from complex type
    		...
    		throw(new DMFeedbackSet("The specified object is not a DmsDefinition object: \n" + def.toOIF()));
        }
     ...
     * @param rule
     */
    public void addRuleData(RuleData rule) throws DMFeedbackSet {
    	...
    	if (existing == null){
    		...
    		throw(new DMFeedbackSet("The following rules have a name clash:\n\n" + existing.toOIF() + "\n" + rule.toOIF()));
    	}
    }
     ...
     * @throws DMFeedbackSet 
     */
    boolean addEnum(EnumDefinition evd)  throws DMFeedbackSet {
        if (checkAndAdd(evd.getObjectName(),evd,enumDefs) == false){
    		throw(new DMFeedbackSet(clashMsg(evd.getObjectName(),evd,enumDefs,"enum value names")));
        }
        
        if (checkAndAddDOT(evd.getDotName(),evd,globallyUniqueMAP,null) == false){
    		throw(new DMFeedbackSet(clashMsgDOT(evd.getObjectName(),evd,globallyUniqueMAP,"definition names")));
        }
     ...
     * @throws DMFeedbackSet 
     */
    void addAction(ActionDefinition actd) throws DMFeedbackSet {
        ...
        if (checkAndAdd(actd.getObjectName(),actd,actionDefs) == false){
    		throw(new DMFeedbackSet(clashMsg(actd.getObjectName(),actd,actionDefs,"action names")));
        }

        if (checkAndAddDOT(actd.getDotName(),actd,globallyUniqueMAP,null) == false){
    		throw(new DMFeedbackSet(clashMsgDOT(actd.getObjectName(),actd,globallyUniqueMAP,"definition names")));
        }
...
//		}
     ...
     * @throws DMFeedbackSet  
     */
    public ClassDefinition cdef(String n) throws DmcNameClashException, DMFeedbackSet {
    	if (n.contains(".")){
    		try {
     ...
     * @return a definition name based on the string.
     */
     ...
     * @throws DmcValueExceptionSet 
     */
	public void resolveReferences(SchemaDefinition sd, DmcNameClashResolverIF clashResolver) throws DMFeedbackSet, DmcNameClashException {
    	...
    	if (ctdl != null){
    		while(ctdl.hasNext()){
    			...
    			try {
					...
					e.setLocation(ctd.getFile(), ctd.getLineNumber());
					throw(e);
				}
    		}
    	}
...
//				}
				...
				if (ertd.getExtendedReferenceClass().getIsNamedBy() == null){
					DMFeedback error = new DMFeedback("The class: " + ertd.getExtendedReferenceClass().getObjectName().getNameString() + " referred to in ExtendedReferenceTypeDefinition " + ertd.getName() + " is not a named object.");
					error.setLocation(ertd.getFile(), ertd.getLineNumber());
					throw(new DMFeedbackSet(error));
				}
    		}
    	}
    	...
    	if (actdl != null){
    		while(actdl.hasNext()){
    			...
    			try {
					...
					DMFeedbackSet ex = new DMFeedbackSet("Unresolved references in ActionDefinition: " + d.getName());
					ex.setLocation(d.getFile(), d.getLineNumber());
					ex.add(e);
					throw(ex);
				}
    		}
    	...
    	if (adl != null){
    		while(adl.hasNext()){
    			...
    			try {
					...
					DMFeedbackSet ex = new DMFeedbackSet("Unresolved references in AttributeDefinition: " + d.getName(),d.getFile(), d.getLineNumber());
					ex.add(e);
					throw(ex);
				}
		        	...
		        	if (d.getType().getKeyClass() == null){
		        		if (d.getType().getIsRefType()){
		        			if (d.getType().getIsExtendedRefType()){
		    					DMFeedbackSet ex = new DMFeedbackSet("AttributeDefinition: " + d.getName() + " cannot have valueType HASHMAPPED/TREEMAPPED since extended reference types are not keyed.",d.getFile(), d.getLineNumber());
				        		throw(ex);
		        			}
		        			if (d.getType().getOriginalClass().getIsNamedBy() == null){
		        				DMFeedbackSet ex = new DMFeedbackSet("AttributeDefinition: " + d.getName() + " cannot have valueType HASHMAPPED/TREEMAPPED since the " + d.getType().getName() + " type does not refer to a named object.",d.getFile(), d.getLineNumber());
				        		throw(ex);
		        			}
		        		}
		        		else{
