...
public class UUIDLite implements Comparable<Object>, Serializable {
	...
	public UUIDLite(String s) throws DMFeedbackSet {
		parseString(s);
		asString 		= null;
		hash			= -1;
	}
	 ...
	 */
	private void parseString(String uuid) throws DMFeedbackSet {
		if (uuid == null) {
		...
		if (i != position.length || lastPosition != -1) {
			throw(new DMFeedbackSet("String does not conform to UUID format: " + uuid));
		}
