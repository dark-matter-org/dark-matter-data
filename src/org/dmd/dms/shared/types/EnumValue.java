...
public class EnumValue {
	 ...
	 */
	public EnumValue(String v) throws DMFeedbackSet {
		this.set(v);
	}
	 ...
	 * value format.
	 */
	public void set(String v) throws DMFeedbackSet {
		if (v == null){
			throw(new DMFeedbackSet("null value passed to EnumValue.set()"));
		}
		...
		if (space == -1)
			throw(new DMFeedbackSet("Missing enum value name: " + value + " - value should be of the form: int enum_val_name [description] <: label>"));
		
		try {
			id = Integer.valueOf(value.substring(0,space));
		} catch (NumberFormatException e) {
			throw(new DMFeedbackSet("Invalid enum integer value: " + value + "\n" + e));
		}
		...
		if (space == -1)
			throw(new DMFeedbackSet("Missing enum description: " + value + " - value should be of the form: int enum_val_name [description] <: label>"));
		...
		if (Character.isLetter(tmp.charAt(0))){
			...
			throw(new DMFeedbackSet("Enum value name must start with a character: " + tmp));
		}
		...
		if (colon == -1){
			...
			if ( (colon+1) == remainder.length()){
				throw(new DMFeedbackSet("Missing label value: " + value + " - value should be of the form: int enum_val_name [description] <: label>"));
			}
...
//			throw(new DMFeedbackSet("Incorrect number of tokens for EnumValue.set(): " + value));
//		}
//		
//		try {
//			id = Integer.valueOf(value.substring(0,space1));
//		} catch (NumberFormatException e) {
//			throw(new DMFeedbackSet("Invalid enum integer value: " + value.substring(space1) + "\n" + e));
//		}
//		
...
//		}
//		else{
//			throw(new DMFeedbackSet("Enum value name must start with a character: " + value.substring(space1+1,space2)));
//		}
//		
