...
public class Modifier implements Serializable {
	 ...
	 */
	public Modifier(String v) throws DMFeedbackSet {
		if (v == null){
			throw(new DMFeedbackSet("Null value passed to Modifier()"));
		}
		...
		if (space1 == -1){
			throw(new DMFeedbackSet("Incorrect number of tokens for Modifier: " + v));
		}
		...
		if (space2 == -1){
			...
			if (operation == null){
				throw(new DMFeedbackSet("Unknown modify operation: " + op));
			}
			
			if (operation != ModifyTypeEnum.REM){
				throw(new DMFeedbackSet("Missing value for modify operation: " + op));
			}
		}
		else{
			...
			if (operation == ModifyTypeEnum.REM){
				throw(new DMFeedbackSet("Extraneous tokens in a REM operation. Should be just " + attributeName + " REM"));
			}
