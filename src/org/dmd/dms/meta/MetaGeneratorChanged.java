 ...
 */
public class MetaGeneratorChanged implements DmcUncheckedOIFHandlerIF {
...
//				GenUtility.dumpSETType(	typedir, 	"org.dmd.dms", 	typedef.getSV("typeClassName"), typedef.getSV("name"), 	typedef.getSV("primitiveType"), null, 			null, 			genericArgs,	false,				LGPL.toString(), System.out);
				...
				if (isFilterType != null){
					...
					nameAttrID = attrDef.getSV("dmdID");
									...
									// 	dmotypedir 	basePackage 	baseTypeImport 					typeName 				dmcTypeImport 					nameAttrImport 	nameAttr	nameAttrID	generic 	 	isRef	isNameType	isFilterType	fileHeader 	  progress
...
//        out = new BufferedWriter(new FileWriter(od + "/MetaSchemaAG.java"));
        ...
        for(int i=0;i<origOrderTypes.size();i++){
        	String tmp = origOrderTypes.get(i);
        	if (tmp.indexOf("Enum") == -1)
                out.write("    public static TypeDefinition      _" + origOrderTypes.get(i) + ";\n");
        	else{
        		if (tmp.equals("EnumValue"))
                    out.write("    public static TypeDefinition      _" + origOrderTypes.get(i) + ";\n");
        		else if (tmp.equals("EnumDefinitionREF"))
                    out.write("    public static TypeDefinition      _" + origOrderTypes.get(i) + ";\n");
        		else
        			out.write("    public static TypeDefinition      _" + origOrderTypes.get(i) + "REF;\n");
        	}
        }
        out.write("\n");
...
//            out.write("            _" + pf.sprintf(defn));
            out.write("            _" + pf.sprintf(predefname));
...
//	            mediatorName = typeName;
		        if (typeDef.get("isEnumType") != null)
		        	mediatorName = typeName + "REF";
		        else
		        	mediatorName = typeName;
	        }
