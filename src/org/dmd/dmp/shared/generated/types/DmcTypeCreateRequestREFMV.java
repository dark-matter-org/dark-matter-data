...
public class DmcTypeCreateRequestREFMV extends DmcTypeCreateRequestREF implements Serializable {
    ...
    public ArrayList<CreateRequestDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<CreateRequestDMO> rc = new  ArrayList<CreateRequestDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(CreateRequestDMO val: value)
                        rc.add((CreateRequestDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (CreateRequestDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public CreateRequestDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<CreateRequestDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
