...
public class DmcTypeCreateResponseREFMV extends DmcTypeCreateResponseREF implements Serializable {
    ...
    public ArrayList<CreateResponseDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<CreateResponseDMO> rc = new  ArrayList<CreateResponseDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(CreateResponseDMO val: value)
                        rc.add((CreateResponseDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (CreateResponseDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public CreateResponseDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<CreateResponseDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
