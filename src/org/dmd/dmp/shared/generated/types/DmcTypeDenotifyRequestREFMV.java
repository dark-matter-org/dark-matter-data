    ...
    public ArrayList<DenotifyRequestDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<DenotifyRequestDMO> rc = new  ArrayList<DenotifyRequestDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(DenotifyRequestDMO val: value)
                        rc.add((DenotifyRequestDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (DenotifyRequestDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
