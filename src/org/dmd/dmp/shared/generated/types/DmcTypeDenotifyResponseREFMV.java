    ...
    public ArrayList<DenotifyResponseDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<DenotifyResponseDMO> rc = new  ArrayList<DenotifyResponseDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(DenotifyResponseDMO val: value)
                        rc.add((DenotifyResponseDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (DenotifyResponseDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
