    ...
    public ArrayList<LogoutResponseDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<LogoutResponseDMO> rc = new  ArrayList<LogoutResponseDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(LogoutResponseDMO val: value)
                        rc.add((LogoutResponseDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (LogoutResponseDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
