...
public class DmcTypeGetResponseREFMV extends DmcTypeGetResponseREF implements Serializable {
    ...
    public ArrayList<GetResponseDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<GetResponseDMO> rc = new  ArrayList<GetResponseDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(GetResponseDMO val: value)
                        rc.add((GetResponseDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (GetResponseDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public GetResponseDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            if (attrInfo.indexSize == 0)
            ...
            if (value == null){
                value = new ArrayList<GetResponseDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
