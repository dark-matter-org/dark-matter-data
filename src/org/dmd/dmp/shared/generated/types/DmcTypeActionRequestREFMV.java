...
public class DmcTypeActionRequestREFMV extends DmcTypeActionRequestREF implements Serializable {
    ...
    public ArrayList<ActionRequestDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<ActionRequestDMO> rc = new  ArrayList<ActionRequestDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(ActionRequestDMO val: value)
                        rc.add((ActionRequestDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (ActionRequestDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public ActionRequestDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            if (attrInfo.indexSize == 0)
            ...
            if (value == null){
                value = new ArrayList<ActionRequestDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
