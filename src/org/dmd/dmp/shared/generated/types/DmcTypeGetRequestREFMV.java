...
public class DmcTypeGetRequestREFMV extends DmcTypeGetRequestREF implements Serializable {
    ...
    public ArrayList<GetRequestDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<GetRequestDMO> rc = new  ArrayList<GetRequestDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(GetRequestDMO val: value)
                        rc.add((GetRequestDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (GetRequestDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public GetRequestDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<GetRequestDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
