    ...
    public ArrayList<PreAuthRequestDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<PreAuthRequestDMO> rc = new  ArrayList<PreAuthRequestDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(PreAuthRequestDMO val: value)
                        rc.add((PreAuthRequestDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (PreAuthRequestDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
