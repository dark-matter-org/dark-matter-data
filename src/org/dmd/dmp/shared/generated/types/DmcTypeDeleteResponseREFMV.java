    ...
    public ArrayList<DeleteResponseDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<DeleteResponseDMO> rc = new  ArrayList<DeleteResponseDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(DeleteResponseDMO val: value)
                        rc.add((DeleteResponseDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (DeleteResponseDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
