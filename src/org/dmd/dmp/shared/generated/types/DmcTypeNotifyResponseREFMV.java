...
public class DmcTypeNotifyResponseREFMV extends DmcTypeNotifyResponseREF implements Serializable {
    ...
    public ArrayList<NotifyResponseDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<NotifyResponseDMO> rc = new  ArrayList<NotifyResponseDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(NotifyResponseDMO val: value)
                        rc.add((NotifyResponseDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (NotifyResponseDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public NotifyResponseDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<NotifyResponseDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
