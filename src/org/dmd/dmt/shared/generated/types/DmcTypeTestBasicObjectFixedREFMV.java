    ...
    public ArrayList<TestBasicObjectFixedDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<TestBasicObjectFixedDMO> rc = new  ArrayList<TestBasicObjectFixedDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(TestBasicObjectFixedDMO val: value)
                        rc.add((TestBasicObjectFixedDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (TestBasicObjectFixedDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
