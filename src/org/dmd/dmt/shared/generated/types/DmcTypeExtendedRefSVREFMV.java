    ...
    public ArrayList<ExtendedRefSVDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<ExtendedRefSVDMO> rc = new  ArrayList<ExtendedRefSVDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(ExtendedRefSVDMO val: value)
                        rc.add((ExtendedRefSVDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (ExtendedRefSVDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
