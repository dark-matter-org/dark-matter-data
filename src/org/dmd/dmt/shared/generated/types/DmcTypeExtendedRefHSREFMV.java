    ...
    public ArrayList<ExtendedRefHSDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<ExtendedRefHSDMO> rc = new  ArrayList<ExtendedRefHSDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(ExtendedRefHSDMO val: value)
                        rc.add((ExtendedRefHSDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (ExtendedRefHSDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
