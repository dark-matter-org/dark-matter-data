    ...
    public ArrayList<PrimitiveHSDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<PrimitiveHSDMO> rc = new  ArrayList<PrimitiveHSDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(PrimitiveHSDMO val: value)
                        rc.add((PrimitiveHSDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (PrimitiveHSDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
