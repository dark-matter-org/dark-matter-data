...
public class DmcTypeUsingIndexedAttributesREFMV extends DmcTypeUsingIndexedAttributesREF implements Serializable {
    
    private final static Iterator<UsingIndexedAttributesDMO> emptyList = (new ArrayList<UsingIndexedAttributesDMO>()).iterator();
    
    protected ArrayList<UsingIndexedAttributesDMO> value;
    
    public DmcTypeUsingIndexedAttributesREFMV(){
    
    }
    
    public DmcTypeUsingIndexedAttributesREFMV(DmcAttributeInfo ai){
        super(ai);
    }
    
    @Override
    public DmcTypeUsingIndexedAttributesREFMV getNew(){
        return(new DmcTypeUsingIndexedAttributesREFMV(attrInfo));
    }
    ...
    public ArrayList<UsingIndexedAttributesDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                return(new ArrayList<UsingIndexedAttributesDMO>());
            else{
                ArrayList<UsingIndexedAttributesDMO> rc = new  ArrayList<UsingIndexedAttributesDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(UsingIndexedAttributesDMO val: value)
                        rc.add((UsingIndexedAttributesDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (UsingIndexedAttributesDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public UsingIndexedAttributesDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<UsingIndexedAttributesDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
