    ...
    public ArrayList<ExtendedRefMVDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<ExtendedRefMVDMO> rc = new  ArrayList<ExtendedRefMVDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(ExtendedRefMVDMO val: value)
                        rc.add((ExtendedRefMVDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (ExtendedRefMVDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
