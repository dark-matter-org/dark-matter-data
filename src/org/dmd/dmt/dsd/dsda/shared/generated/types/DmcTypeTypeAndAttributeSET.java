...
public class DmcTypeTypeAndAttributeSET extends DmcTypeTypeAndAttribute implements Serializable {
    
     private final static Iterator<TypeAndAttribute> emptyList =  (new HashSet<TypeAndAttribute>()).iterator();
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<TypeAndAttribute>();
        else
            value = new TreeSet<TypeAndAttribute>();
    }
    
    @Override
    public DmcTypeTypeAndAttributeSET getNew(){
        return(new DmcTypeTypeAndAttributeSET(getAttributeInfo()));
    }
    ...
    public DmcAttribute<TypeAndAttribute> cloneIt(){
        synchronized(this){
            DmcTypeTypeAndAttributeSET rc = getNew();
    ...
    public Iterator<TypeAndAttribute> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<TypeAndAttribute>(value)).iterator() );
            else
                return( (new TreeSet<TypeAndAttribute>(value)).iterator() );
        }
    }
        ...
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    ...
                    return(new HashSet<TypeAndAttribute>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<TypeAndAttribute>(value));
                else
                    return(new TreeSet<TypeAndAttribute>(value));
            }
        }
