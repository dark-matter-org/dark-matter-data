...
public class DmcTypeModuleBREFSET extends DmcTypeModuleBREF implements Serializable {
    
     private final static Iterator<ModuleBREF> emptyList =  (new HashSet<ModuleBREF>()).iterator();
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<ModuleBREF>();
        else
            value = new TreeSet<ModuleBREF>();
    }
    
    @Override
    public DmcTypeModuleBREFSET getNew(){
        return(new DmcTypeModuleBREFSET(getAttributeInfo()));
    }
    ...
    public DmcAttribute<ModuleBREF> cloneIt(){
        synchronized(this){
            DmcTypeModuleBREFSET rc = getNew();
    ...
    public Iterator<ModuleBREF> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<ModuleBREF>(value)).iterator() );
            else
                return( (new TreeSet<ModuleBREF>(value)).iterator() );
        }
    }
        ...
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    ...
                    return(new HashSet<ModuleBREF>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<ModuleBREF>(value));
                else
                    return(new TreeSet<ModuleBREF>(value));
            }
        }
