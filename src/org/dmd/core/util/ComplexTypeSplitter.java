...
public class ComplexTypeSplitter {
	...
	static public ArrayList<ParsedNameValuePair> parse(String initialInput) throws DMFeedbackSet {
		return(parse(initialInput,' '));
	}

	static public ArrayList<ParsedNameValuePair> parse(String initialInput, char separator) throws DMFeedbackSet {
		return(parse(initialInput, separator, -1));
	}
	
	static public ArrayList<ParsedNameValuePair> parse(String initialInput, char separator, int stopAfter) throws DMFeedbackSet {
        if (initialInput == null){
        	throw(new DMFeedbackSet("No content to parse!"));
        }
        ...
        for(int i=0; i<input.length(); i++){
        	if (input.charAt(i) == '"'){
        		...
        		throw(new DMFeedbackSet("Missing name for value"));
        	}
			else{
				position.set(i);
	...
	static private ParsedNameValuePair parseQuotedText(String input, String namePart, IntegerVar position) throws DMFeedbackSet {
		...
		if ((position.intValue() + 1) == input.length())
			throw(new DMFeedbackSet("Unmatched \"" + showErrorLocation(position.intValue()+1, input)));
		...
		if (rc == null)
			throw(new DMFeedbackSet("Unmatched \"" + showErrorLocation(input.length(), input)));
	 ...
	 */
	static private ParsedNameValuePair parsePart(String input, IntegerVar position, char separator, int stopAfter, int nvpCount) throws DMFeedbackSet {
		ParsedNameValuePair 	rc				= null;
		...
		for(int i=startPos; i<input.length(); i++){
			...
			if (input.charAt(i) == '"'){
				if (namePart == null){
					...
					throw(new DMFeedbackSet("Misplaced double quote (or missing double quote), expecting part of a name or an equals sign." + showErrorLocation(i, input)));
				}
				else if (!haveEquals){
					throw(new DMFeedbackSet("Missing = for value assignment." + showErrorLocation(i, input)));
				}
				...
				if (i == startPos){
					...
					if (haveEquals){
						if (i > afterEqualsPos){
							...
							// For example: version= 
							throw(new DMFeedbackSet("Missing value after equals (=) - no spaces allowed on either side of equals." + showErrorLocation(i, input)));
						}
					}
				}
			}
			...
			if (namePart == null){
				...
				if (haveEquals){
					if (afterEqualsPos >= input.length()){
						...
						throw(new DMFeedbackSet("Missing value after = " + showErrorLocation(input.length(), input)));
					}
					else{
