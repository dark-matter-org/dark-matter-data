...
public class DmcTypeGxtEnumMappingREFMAP extends DmcTypeGxtEnumMappingREF implements Serializable {
    ...
    private final static Iterator<GxtEnumMappingREF> emptyList = (new HashMap<CamelCaseName,GxtEnumMappingREF>()).values().iterator();
    ...
    public Map<CamelCaseName,GxtEnumMappingREF> getMVCopy(){
        synchronized(this){
            ...
            if (attrInfo.valueType == ValueTypeEnum.HASHMAPPED){
                if (value == null)
                    clone = new HashMap<CamelCaseName,GxtEnumMappingREF>();
                else
                    clone = new HashMap<CamelCaseName,GxtEnumMappingREF>(value);
            }
            else{
                if (value == null)
                    clone = new TreeMap<CamelCaseName,GxtEnumMappingREF>();
                else
                    clone = new TreeMap<CamelCaseName,GxtEnumMappingREF>(value);
            }
            return(clone);
