...
public class DmcTypeFormBindingDefinitionREFMV extends DmcTypeFormBindingDefinitionREF implements Serializable {
    ...
    private final static Iterator<FormBindingDefinitionREF> emptyList = (new ArrayList<FormBindingDefinitionREF>()).iterator();
    
    protected ArrayList<FormBindingDefinitionREF> value;
    
    public DmcTypeFormBindingDefinitionREFMV(){
    
    }
    
    public DmcTypeFormBindingDefinitionREFMV(DmcAttributeInfo ai){
        super(ai);
    }
    
    @Override
    public DmcTypeFormBindingDefinitionREFMV getNew(){
        return(new DmcTypeFormBindingDefinitionREFMV(attrInfo));
    }
    ...
    public FormBindingDefinitionREF add(Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null)
                value = new ArrayList<FormBindingDefinitionREF>();
            value.add(rc);
            return(rc);
        }
    }
    ...
    public ArrayList<FormBindingDefinitionREF> getMVCopy(){
        synchronized(this){
            if (value == null)
                return(new ArrayList<FormBindingDefinitionREF>());
            else{
                ArrayList<FormBindingDefinitionREF> rc = new  ArrayList<FormBindingDefinitionREF>(value.size());
                if (attrInfo.indexSize == 0){
                    for(FormBindingDefinitionREF val: value)
                        rc.add(val);
                }
    ...
    public FormBindingDefinitionREF setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<FormBindingDefinitionREF>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
