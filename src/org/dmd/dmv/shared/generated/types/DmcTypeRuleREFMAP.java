...
public class DmcTypeRuleREFMAP extends DmcTypeRuleREF implements Serializable {
    ...
    private final static Iterator<RuleREF> emptyList = (new HashMap<StringName,RuleREF>()).values().iterator();
    ...
    public Map<StringName,RuleREF> getMVCopy(){
        synchronized(this){
            ...
            if (attrInfo.valueType == ValueTypeEnum.HASHMAPPED){
                if (value == null)
                    clone = new HashMap<StringName,RuleREF>();
                else
                    clone = new HashMap<StringName,RuleREF>(value);
            }
            else{
                if (value == null)
                    clone = new TreeMap<StringName,RuleREF>();
                else
                    clone = new TreeMap<StringName,RuleREF>(value);
            }
            return(clone);
