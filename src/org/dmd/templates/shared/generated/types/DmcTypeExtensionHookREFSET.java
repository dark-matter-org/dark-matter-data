...
public class DmcTypeExtensionHookREFSET extends DmcTypeExtensionHookREF implements Serializable {
     ...
     private final static Iterator<ExtensionHookREF> emptyList =  (new HashSet<ExtensionHookREF>()).iterator();
    
    
    protected Set<ExtensionHookREF> value;
    
    public DmcTypeExtensionHookREFSET(){
        value = null;
    }
    
    public DmcTypeExtensionHookREFSET(DmcAttributeInfo ai){
        super(ai);
        initValue();
    }
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<ExtensionHookREF>();
        else
            value = new TreeSet<ExtensionHookREF>();
    }
    
    @Override
    public DmcTypeExtensionHookREFSET getNew(){
        return(new DmcTypeExtensionHookREFSET(getAttributeInfo()));
    }
    ...
    public Iterator<ExtensionHookREF> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<ExtensionHookREF>(value)).iterator() );
            else
                return( (new TreeSet<ExtensionHookREF>(value)).iterator() );
        }
    }
    ...
    public Set<ExtensionHookREF> getMVCopy(){
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    return(new HashSet<ExtensionHookREF>());
                else
                    return(new HashSet<ExtensionHookREF>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<ExtensionHookREF>(value));
                else
                    return(new TreeSet<ExtensionHookREF>(value));
            }
        }
