...
public class DmcTypeTemplateREFMAP extends DmcTypeTemplateREF implements Serializable {
    
    private final static Iterator<TemplateREF> emptyList = (new HashMap<DefinitionName,TemplateREF>()).values().iterator();
    
    protected Map<DefinitionName,TemplateREF> value;
    
    public DmcTypeTemplateREFMAP(){
        value = null;
    }
    
    public DmcTypeTemplateREFMAP(DmcAttributeInfo ai){
        super(ai);
        initValue();
    }
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHMAPPED)
            value = new HashMap<DefinitionName,TemplateREF>();
        else
            value = new TreeMap<DefinitionName,TemplateREF>();
    }
    ...
    public DefinitionName firstKey(){
        if (getAttributeInfo().valueType == ValueTypeEnum.TREEMAPPED){
            if (value == null)
                return(null);
            TreeMap<DefinitionName,TemplateREF> map = (TreeMap<DefinitionName,TemplateREF>)value;
            return(map.firstKey());
        }
    ...
    @Override
    public DmcTypeTemplateREFMAP getNew(){
        return(new DmcTypeTemplateREFMAP(getAttributeInfo()));
    }
    ...
    public DmcAttribute<TemplateREF> cloneIt(){
        synchronized(this){
            ...
            for(TemplateREF val: value.values())
            try {
                rc.add(val);
    ...
    public TemplateREF add(Object v) throws DmcValueException {
        synchronized(this){
            ...
            TemplateREF oldval = value.put(key,newval);
    ...
    public Iterator<TemplateREF> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHMAPPED)
                clone = new HashMap<DefinitionName,TemplateREF>(value);
            else
                clone = new TreeMap<DefinitionName,TemplateREF>(value);
    ...
    public Map<DefinitionName,TemplateREF> getMVCopy(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHMAPPED){
                if (value == null)
                    clone = new HashMap<DefinitionName,TemplateREF>();
                else
                    clone = new HashMap<DefinitionName,TemplateREF>(value);
            }
            else{
                if (value == null)
                    clone = new TreeMap<DefinitionName,TemplateREF>();
                else
                    clone = new TreeMap<DefinitionName,TemplateREF>(value);
            }
            return(clone);
