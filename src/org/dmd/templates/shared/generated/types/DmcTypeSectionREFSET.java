...
public class DmcTypeSectionREFSET extends DmcTypeSectionREF implements Serializable {
    
     private final static Iterator<SectionREF> emptyList =  (new HashSet<SectionREF>()).iterator();
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<SectionREF>();
        else
            value = new TreeSet<SectionREF>();
    }
    
    @Override
    public DmcTypeSectionREFSET getNew(){
        return(new DmcTypeSectionREFSET(getAttributeInfo()));
    }
    ...
    public DmcAttribute<SectionREF> cloneIt(){
        synchronized(this){
            DmcTypeSectionREFSET rc = getNew();
    ...
    public Iterator<SectionREF> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<SectionREF>(value)).iterator() );
            else
                return( (new TreeSet<SectionREF>(value)).iterator() );
        }
    }
        ...
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    ...
                    return(new HashSet<SectionREF>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<SectionREF>(value));
                else
                    return(new TreeSet<SectionREF>(value));
            }
        }
