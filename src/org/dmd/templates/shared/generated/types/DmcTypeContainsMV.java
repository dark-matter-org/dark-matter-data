...
public class DmcTypeContainsMV extends DmcTypeContains implements Serializable {
    
    private final static Iterator<Contains> emptyList = (new ArrayList<Contains>()).iterator();
    
    protected ArrayList<Contains> value;
    
    public DmcTypeContainsMV(){
    
    }
    
    public DmcTypeContainsMV(DmcAttributeInfo ai){
        super(ai);
    }
    
    @Override
    public DmcTypeContainsMV getNew(){
        return(new DmcTypeContainsMV(getAttributeInfo()));
    }
    ...
    public DmcAttribute<Contains> cloneIt(){
        synchronized(this){
            DmcTypeContainsMV rc = getNew();
    
            if (value == null)
                return(rc);
    
            if (getAttributeInfo().indexSize == 0){
                for(Contains val: value)
                try {
                    rc.add(val);
    ...
    public Iterator<Contains> getMV(){
        synchronized(this){
            ...
            ArrayList<Contains> clone = new ArrayList<Contains>(value);
            return(clone.iterator());
        }
    }
    
    // org.dmd.dms.util.GenUtility.dumpMVType(GenUtility.java:2618)
    public ArrayList<Contains> getMVCopy(){
        synchronized(this){
            if (value == null)
                return(new ArrayList<Contains>());
            else 
                return(new ArrayList<Contains>(value));
        }
    }
    ...
    public Contains getMVnth(int index){
        synchronized(this){
            if (value == null)
                return(null);
    ...
    public Contains setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            if (getAttributeInfo().indexSize == 0)
            ...
            if (value == null){
                value = new ArrayList<Contains>(getAttributeInfo().indexSize);
