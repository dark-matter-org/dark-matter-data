...
public class DmcTypePathAndKeySET extends DmcTypePathAndKey implements Serializable {
    
     private final static Iterator<PathAndKey> emptyList =  (new HashSet<PathAndKey>()).iterator();
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<PathAndKey>();
        else
            value = new TreeSet<PathAndKey>();
    }
    
    @Override
    public DmcTypePathAndKeySET getNew(){
        return(new DmcTypePathAndKeySET(getAttributeInfo()));
    }
    ...
    public DmcAttribute<PathAndKey> cloneIt(){
        synchronized(this){
            DmcTypePathAndKeySET rc = getNew();
    
            if (value == null)
                return(rc);
    
            for(PathAndKey val: value)
            try {
                rc.add(val);
    ...
    public Iterator<PathAndKey> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<PathAndKey>(value)).iterator() );
            else
                return( (new TreeSet<PathAndKey>(value)).iterator() );
        }
    }
        ...
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    ...
                    return(new HashSet<PathAndKey>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<PathAndKey>(value));
                else
                    return(new TreeSet<PathAndKey>(value));
            }
        }
