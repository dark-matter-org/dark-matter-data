...
// public class DmcTypeDmuDefinitionREFMAP extends DmcTypeDmuDefinitionREF<DmuDefinitionREF,DefinitionName> {
public class DmcTypeDmuDefinitionREFMAP extends DmcTypeDmuDefinitionREF implements Serializable {
    
    private final static Iterator<DmuDefinitionREF> emptyList = (new HashMap<DefinitionName,DmuDefinitionREF>()).values().iterator();
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHMAPPED)
            value = new HashMap<DefinitionName,DmuDefinitionREF>();
        else
            value = new TreeMap<DefinitionName,DmuDefinitionREF>();
    }
    ...
    public DefinitionName firstKey(){
        if (getAttributeInfo().valueType == ValueTypeEnum.TREEMAPPED){
            if (value == null)
                return(null);
            TreeMap<DefinitionName,DmuDefinitionREF> map = (TreeMap<DefinitionName,DmuDefinitionREF>)value;
            return(map.firstKey());
        }
    ...
    public DmcTypeDmuDefinitionREFMAP getNew(){
        return(new DmcTypeDmuDefinitionREFMAP(getAttributeInfo()));
    }
    ...
    public DmcAttribute<DmuDefinitionREF> cloneIt(){
        synchronized(this){
            DmcTypeDmuDefinitionREFMAP rc = getNew();
    
            if (value == null)
                return(rc);
    
            for(DmuDefinitionREF val: value.values())
            try {
                rc.add(val);
    ...
    public DmuDefinitionREF add(Object v) throws DmcValueException {
        synchronized(this){
            DmuDefinitionREF newval = typeCheck(v);
            if (value == null)
                initValue();
            DefinitionName key = (DefinitionName)((DmcMappedAttributeIF)newval).getKey();
            DmuDefinitionREF oldval = value.put(key,newval);
    ...
    public Iterator<DmuDefinitionREF> getMV(){
        synchronized(this){
            ...
            Map<DefinitionName,DmuDefinitionREF> clone = null;
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHMAPPED)
                clone = new HashMap<DefinitionName,DmuDefinitionREF>(value);
            else
                clone = new TreeMap<DefinitionName,DmuDefinitionREF>(value);
    ...
    public Map<DefinitionName,DmuDefinitionREF> getMVCopy(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHMAPPED){
                if (value == null)
                    clone = new HashMap<DefinitionName,DmuDefinitionREF>();
                else
                    clone = new HashMap<DefinitionName,DmuDefinitionREF>(value);
            }
            else{
                if (value == null)
                    clone = new TreeMap<DefinitionName,DmuDefinitionREF>();
                else
                    clone = new TreeMap<DefinitionName,DmuDefinitionREF>(value);
            }
            return(clone);
    ...
    public boolean contains(Object v){
        synchronized(this){
           if (value == null)
            ...
            try {
                DmuDefinitionREF val = typeCheck(v);
