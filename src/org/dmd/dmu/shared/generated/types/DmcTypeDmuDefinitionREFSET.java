...
public class DmcTypeDmuDefinitionREFSET extends DmcTypeDmuDefinitionREF implements Serializable {
    
     private final static Iterator<DmuDefinitionREF> emptyList =  (new HashSet<DmuDefinitionREF>()).iterator();
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<DmuDefinitionREF>();
        else
            value = new TreeSet<DmuDefinitionREF>();
    }
    
    @Override
    public DmcTypeDmuDefinitionREFSET getNew(){
        return(new DmcTypeDmuDefinitionREFSET(getAttributeInfo()));
    }
    ...
    public DmcAttribute<DmuDefinitionREF> cloneIt(){
        synchronized(this){
            DmcTypeDmuDefinitionREFSET rc = getNew();
    ...
    public Iterator<DmuDefinitionREF> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<DmuDefinitionREF>(value)).iterator() );
            else
                return( (new TreeSet<DmuDefinitionREF>(value)).iterator() );
        }
    }
        ...
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    ...
                    return(new HashSet<DmuDefinitionREF>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<DmuDefinitionREF>(value));
                else
                    return(new TreeSet<DmuDefinitionREF>(value));
            }
        }
