...
abstract public class SecurityControllerMVC extends Controller implements ServerEventHandlerIF {
    ...
    // MultiView(s)
    protected TreeMap<String,UserView> userViews;
    ...
    protected SecurityControllerMVC(){
        ...
        userViews = new TreeMap<String,UserView>();

        // Instantiate our actions
        actionLogout = new ActionLogout(this);

        // Instantiate our menus
        secMenu = new DefaultToolbarSubmenu("sec.menu", "50", "common.mainMenuBar", "Security");

        // Instantiate our menu items
        secMenuLogout = new DefaultMenuItem("sec.menu.logout","10","action.logout","sec.menu","Logout");
    ...
    public UserView createUserView(String instanceName){
        ...
        userViews.put(instanceName,rc);
        return(rc);
    }

    public UserView getUserView(String instanceName){
        return(userViews.get(instanceName));
    }

    public UserView removeUserView(String instanceName){
        return(userViews.remove(instanceName));
    }

    public void handleServerEvent(DMPEventDMO event) {
        if (event.getSourceObjectClass().equals("UserGroup"))
            handleSecUserGroupUpdate(event.getEventTypeDMP(),(UserGroupDMO)event.getSourceObject());
    }
