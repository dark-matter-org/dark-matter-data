...
public class DmcTypePlaceREFMV extends DmcTypePlaceREF implements Serializable {
    ...
    private final static Iterator<PlaceREF> emptyList = (new ArrayList<PlaceREF>()).iterator();
    ...
    public ArrayList<PlaceREF> getMVCopy(){
        synchronized(this){
            if (value == null)
                return(new ArrayList<PlaceREF>());
            else{
                ArrayList<PlaceREF> rc = new  ArrayList<PlaceREF>(value.size());
                if (attrInfo.indexSize == 0){
                    for(PlaceREF val: value)
                        rc.add(val);
                }
