...
public class DmcTypeSetRequestREFMV extends DmcTypeSetRequestREF implements Serializable {
    ...
    public ArrayList<SetRequestDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<SetRequestDMO> rc = new  ArrayList<SetRequestDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(SetRequestDMO val: value)
                        rc.add((SetRequestDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (SetRequestDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public SetRequestDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<SetRequestDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
