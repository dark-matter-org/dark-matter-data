...
public class DmcTypeLoginRequestREFMV extends DmcTypeLoginRequestREF implements Serializable {
    ...
    public ArrayList<LoginRequestDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<LoginRequestDMO> rc = new  ArrayList<LoginRequestDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(LoginRequestDMO val: value)
                        rc.add((LoginRequestDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (LoginRequestDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public LoginRequestDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<LoginRequestDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
