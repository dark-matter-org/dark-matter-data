...
/**
 * The DmcTypeMenuREFMAP provides storage for a map of MenuREF
 * <P>
...
// public class DmcTypeMenuREFMAP extends DmcTypeMenuREF<MenuREF,CamelCaseName> {
public class DmcTypeMenuREFMAP extends DmcTypeMenuREF implements Serializable {
    
    private final static Iterator<MenuREF> emptyList = (new HashMap<CamelCaseName,MenuREF>()).values().iterator();
    
    protected Map<CamelCaseName,MenuREF> value;
    
    public DmcTypeMenuREFMAP(){
        value = null;
    }
    
    public DmcTypeMenuREFMAP(DmcAttributeInfo ai){
        super(ai);
        initValue();
    }
    ...
    void initValue(){
        if (attrInfo.valueType == ValueTypeEnum.HASHMAPPED)
            value = new HashMap<CamelCaseName,MenuREF>();
        else
            value = new TreeMap<CamelCaseName,MenuREF>();
    }
    ...
    public CamelCaseName firstKey(){
        if (attrInfo.valueType == ValueTypeEnum.TREEMAPPED){
            if (value == null)
                return(null);
            TreeMap<CamelCaseName,MenuREF> map = (TreeMap<CamelCaseName,MenuREF>)value;
            return(map.firstKey());
        }
    ...
    public DmcTypeMenuREFMAP getNew(){
        return(new DmcTypeMenuREFMAP(attrInfo));
    }
    ...
    public DmcAttribute<MenuREF> cloneIt(){
        synchronized(this){
            DmcTypeMenuREFMAP rc = getNew();
    
            if (value == null)
                return(rc);
    
            for(MenuREF val: value.values())
            try {
                rc.add(val);
    ...
    public Iterator<MenuREF> getMV(){
        synchronized(this){
            ...
            Map<CamelCaseName,MenuREF> clone = null;
            if (attrInfo.valueType == ValueTypeEnum.HASHMAPPED)
                clone = new HashMap<CamelCaseName,MenuREF>(value);
            else
                clone = new TreeMap<CamelCaseName,MenuREF>(value);
            return(clone.values().iterator());
        }
    }
    
    // org.dmd.dms.util.GenUtility.dumpMAPType(GenUtility.java:2931)
    public Map<CamelCaseName,MenuREF> getMVCopy(){
        synchronized(this){
            Map<CamelCaseName,MenuREF> clone = null;
            if (attrInfo.valueType == ValueTypeEnum.HASHMAPPED){
                if (value == null)
                    clone = new HashMap<CamelCaseName,MenuREF>();
                else
                    clone = new HashMap<CamelCaseName,MenuREF>(value);
            }
            else{
                if (value == null)
                    clone = new TreeMap<CamelCaseName,MenuREF>();
                else
                    clone = new TreeMap<CamelCaseName,MenuREF>(value);
            }
            return(clone);
    ...
    public MenuREF getByKey(Object key){
        synchronized(this){
           if (value == null)
               return(null);
