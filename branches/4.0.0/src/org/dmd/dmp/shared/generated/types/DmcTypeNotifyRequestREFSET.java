...
import org.dmd.dmp.shared.generated.dmo.NotifyRequestDMO;         // DmcType import - (SETTypeFormatter.java:59)
...
public class DmcTypeNotifyRequestREFSET extends DmcTypeNotifyRequestREF implements Serializable {
    
     private final static Iterator<NotifyRequestDMO> emptyList =  (new HashSet<NotifyRequestDMO>()).iterator();
    
    
    protected Set<NotifyRequestDMO> value;
    
    public DmcTypeNotifyRequestREFSET(){
        value = null;
    }
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<NotifyRequestDMO>();
        else
            value = new TreeSet<NotifyRequestDMO>();
    }
    
    @Override
    public DmcTypeNotifyRequestREFSET getNew(){
        return(new DmcTypeNotifyRequestREFSET(getAttributeInfo()));
    }
    ...
    public DmcAttribute<NotifyRequestDMO> cloneIt(){
        synchronized(this){
            DmcTypeNotifyRequestREFSET rc = getNew();
    ...
    public NotifyRequestDMO add(Object v) throws DMFeedbackSet {
        synchronized(this){
    ...
    public Iterator<NotifyRequestDMO> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<NotifyRequestDMO>(value)).iterator() );
            else
                return( (new TreeSet<NotifyRequestDMO>(value)).iterator() );
        }
    }
    
    // org.dmd.dms.tools.generation.util.SETTypeFormatter.dumpSETType(SETTypeFormatter.java:187)
    public Set<NotifyRequestDMO> getMVCopy(){
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    return(new HashSet<NotifyRequestDMO>());
                else
                    return(new HashSet<NotifyRequestDMO>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<NotifyRequestDMO>(value));
                else
                    return(new TreeSet<NotifyRequestDMO>(value));
            }
        }
