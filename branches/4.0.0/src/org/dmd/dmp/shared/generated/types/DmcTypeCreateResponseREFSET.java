...
import org.dmd.dmp.shared.generated.dmo.CreateResponseDMO;         // DmcType import - (SETTypeFormatter.java:59)
...
public class DmcTypeCreateResponseREFSET extends DmcTypeCreateResponseREF implements Serializable {
     ...
     private final static Iterator<CreateResponseDMO> emptyList =  (new HashSet<CreateResponseDMO>()).iterator();
    ...
    public DmcTypeCreateResponseREFSET getNew(){
        return(new DmcTypeCreateResponseREFSET(getAttributeInfo()));
    }
    ...
    public Set<CreateResponseDMO> getMVCopy(){
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    return(new HashSet<CreateResponseDMO>());
                else
