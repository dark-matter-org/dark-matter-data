...
import org.dmd.dmp.shared.generated.dmo.LoginResponseDMO;         // DmcType import - (SETTypeFormatter.java:59)
...
public class DmcTypeLoginResponseREFSET extends DmcTypeLoginResponseREF implements Serializable {
    
     private final static Iterator<LoginResponseDMO> emptyList =  (new HashSet<LoginResponseDMO>()).iterator();
    
    
    protected Set<LoginResponseDMO> value;
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<LoginResponseDMO>();
        else
            value = new TreeSet<LoginResponseDMO>();
    }
    
    @Override
    public DmcTypeLoginResponseREFSET getNew(){
        return(new DmcTypeLoginResponseREFSET(getAttributeInfo()));
    }
    ...
    public LoginResponseDMO add(Object v) throws DMFeedbackSet {
        synchronized(this){
    ...
    public Iterator<LoginResponseDMO> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<LoginResponseDMO>(value)).iterator() );
            else
                return( (new TreeSet<LoginResponseDMO>(value)).iterator() );
        }
    }
    
    // org.dmd.dms.tools.generation.util.SETTypeFormatter.dumpSETType(SETTypeFormatter.java:187)
    public Set<LoginResponseDMO> getMVCopy(){
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    return(new HashSet<LoginResponseDMO>());
                else
                    return(new HashSet<LoginResponseDMO>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<LoginResponseDMO>(value));
                else
                    return(new TreeSet<LoginResponseDMO>(value));
            }
        }
