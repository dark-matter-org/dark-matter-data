...
//	final static String dotCase = "[a-zA-Z][a-zA-Z0-9_.]*";
	...
	public DotName(String n) throws DMFeedbackSet {
		if (n.matches(dotCase))
			name = n;
		else
			throw(new DMFeedbackSet("The string: " + n + " does not conform to dot name format."));
	}
	 ...
	 */
	public DotName(DotName p, String child) throws DMFeedbackSet {
		if (child.matches(nameCase)){
			name = new String(p.name + "." + child);
			parent = p;
		}
		else
			throw(new DMFeedbackSet("The string: " + child + " does not conform to dot name format."));
	}
...
//	public void addChild(String c) throws DMFeedbackSet {
//		if (c.matches(nameCase)){
//			name = name + "." + c;
//		}
//		else{
//			throw(new DMFeedbackSet("The string: " + c + " must start with an alpha character followed by alphanumerics."));			
//		}
//	}
