...
import org.dmd.dms.shared.generated.types.DmcTypeExample;         // DmcType import - (SETTypeFormatter.java:60)
import org.dmd.dms.shared.generated.types.Example;                // Base type import - (SETTypeFormatter.java:57)
...
public class DmcTypeExampleSET extends DmcTypeExample implements Serializable {
    
     private final static Iterator<Example> emptyList =  (new HashSet<Example>()).iterator();
    
    
    protected Set<Example> value;
    
    public DmcTypeExampleSET(){
        value = null;
    }
    
    public DmcTypeExampleSET(DmcAttributeInfo ai){
        super(ai);
        initValue();
    }
    ...
    void initValue(){
        if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
            value = new HashSet<Example>();
        else
            value = new TreeSet<Example>();
    }
    
    @Override
    public DmcTypeExampleSET getNew(){
        return(new DmcTypeExampleSET(getAttributeInfo()));
    }
    ...
    public Iterator<Example> getMV(){
        synchronized(this){
            ...
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET)
                return( (new HashSet<Example>(value)).iterator() );
            else
                return( (new TreeSet<Example>(value)).iterator() );
        }
    }
    
    // org.dmd.dms.tools.generation.util.SETTypeFormatter.dumpSETType(SETTypeFormatter.java:188)
    public Set<Example> getMVCopy(){
        synchronized(this){
            if (getAttributeInfo().valueType == ValueTypeEnum.HASHSET){
                if (value == null)
                    return(new HashSet<Example>());
                else
                    return(new HashSet<Example>(value));
            }
            else{
                if (value == null)
                    return(new TreeSet<Example>(value));
                else
                    return(new TreeSet<Example>(value));
            }
        }
