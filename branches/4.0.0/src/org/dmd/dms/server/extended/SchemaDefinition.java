...
import org.dmd.core.rules.DynamicInitIF;
import org.dmd.core.rules.RuleIF;
import org.dmd.core.util.DMUncheckedObject;
import org.dmd.dms.server.generated.dmw.SchemaDefinitionDMW;
import org.dmd.dms.server.util.DmoObjectFactory;
import org.dmd.dms.server.util.DynamicCompactSchema;
...
public class SchemaDefinition extends SchemaDefinitionDMW {
    ...
    TreeMap<RuleName,DMUncheckedObject>	parsedRules;
	...
	protected SchemaDefinition(String mn) throws DMFeedbackSet {
//		super(mn);
		getDMO().setName(mn);
    	dependsOnSchemaClasses = new TreeMap<String, String>();
    	generatedSchema = false;
	 ...
	 */
	public SchemaDefinition getInstance() throws DMFeedbackSet {
		return(null);
	}
	...
	public String getDMSASGImport(){
		String name = Manipulator.dotNameToCamelCase(getName().getNameString()) + "DMSAG";
...
//        DmdGenericAttribute p = (DmdGenericAttribute)pred.getAttr(MetaSchema._typeDefList);
     ...
     * @throws DMFeedbackSet 
     */
    public void addDefinition(DmsDefinition def) throws DMFeedbackSet {
    	...
    	if (def instanceof AttributeDefinition)
        	...
        	DMFeedbackSet ex = new DMFeedbackSet("The specified object is not a DMD object: \n" + def.toOIF());
        	throw(ex);
        }
    }
    
    public Iterator<DMUncheckedObject>	getParsedRules(){
    	if (parsedRules == null)
    		return(null);
    ...
    public Iterator<RuleDataDMO> getParsedRulesDMOs(SchemaManager sm) throws DmcNameClashException {
    	if (parsedRulesDMOs == null){
    		...
    		if (parsedRules != null){
    			...
    			for(DMUncheckedObject uco: parsedRules.values()){
    				try {
    ...
    public void addParsedRule(DMUncheckedObject uco) throws DMFeedbackSet {
    	String ruleName = uco.getSV("ruleName");
    	
    	if (ruleName == null){
    		throw(new DMFeedbackSet("The specified rule instance doesn't have a ruleName: \n" + uco.toOIF()));
    	}
    	
    	if (parsedRules == null)
    		parsedRules = new TreeMap<RuleName, DMUncheckedObject>();
    	
    	RuleName rn = new RuleName(ruleName);
    	
    	DMUncheckedObject existing = parsedRules.get(rn);
    	
    	if (existing != null){
    		throw(new DMFeedbackSet("Clashing ruleNames: \n\n" + existing.toOIF() + "\n" + uco.toOIF()));
    	}
