...
public class DmcTypePluginConfigREFMV extends DmcTypePluginConfigREF implements Serializable {
    ...
    private final static Iterator<PluginConfigREF> emptyList = (new ArrayList<PluginConfigREF>()).iterator();
    
    protected ArrayList<PluginConfigREF> value;
    ...
    public ArrayList<PluginConfigREF> getMVCopy(){
        synchronized(this){
            if (value == null)
                return(new ArrayList<PluginConfigREF>());
            else{
                ArrayList<PluginConfigREF> rc = new  ArrayList<PluginConfigREF>(value.size());
                if (attrInfo.indexSize == 0){
                    for(PluginConfigREF val: value)
                        rc.add(val);
                }
    ...
    public PluginConfigREF setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<PluginConfigREF>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
