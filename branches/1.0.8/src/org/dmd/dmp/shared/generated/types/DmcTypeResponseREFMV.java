...
public class DmcTypeResponseREFMV extends DmcTypeResponseREF implements Serializable {
    ...
    public ArrayList<ResponseDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<ResponseDMO> rc = new  ArrayList<ResponseDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(ResponseDMO val: value)
                        rc.add((ResponseDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (ResponseDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public ResponseDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            if (attrInfo.indexSize == 0)
            ...
            if (value == null){
                value = new ArrayList<ResponseDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
