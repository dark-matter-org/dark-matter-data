    ...
    public ArrayList<DeleteRequestDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<DeleteRequestDMO> rc = new  ArrayList<DeleteRequestDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(DeleteRequestDMO val: value)
                        rc.add((DeleteRequestDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (DeleteRequestDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
