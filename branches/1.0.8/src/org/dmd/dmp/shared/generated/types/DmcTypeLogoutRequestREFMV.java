    ...
    public ArrayList<LogoutRequestDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<LogoutRequestDMO> rc = new  ArrayList<LogoutRequestDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(LogoutRequestDMO val: value)
                        rc.add((LogoutRequestDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (LogoutRequestDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
