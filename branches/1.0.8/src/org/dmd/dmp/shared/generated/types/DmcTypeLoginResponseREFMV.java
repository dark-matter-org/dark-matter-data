    ...
    public ArrayList<LoginResponseDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<LoginResponseDMO> rc = new  ArrayList<LoginResponseDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(LoginResponseDMO val: value)
                        rc.add((LoginResponseDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (LoginResponseDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
