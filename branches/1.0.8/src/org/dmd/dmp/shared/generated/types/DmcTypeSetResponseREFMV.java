    ...
    public ArrayList<SetResponseDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<SetResponseDMO> rc = new  ArrayList<SetResponseDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(SetResponseDMO val: value)
                        rc.add((SetResponseDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (SetResponseDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
