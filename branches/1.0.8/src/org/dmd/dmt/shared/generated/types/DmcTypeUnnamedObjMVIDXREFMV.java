    ...
    public ArrayList<UnnamedObjMVIDXDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<UnnamedObjMVIDXDMO> rc = new  ArrayList<UnnamedObjMVIDXDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(UnnamedObjMVIDXDMO val: value)
                        rc.add((UnnamedObjMVIDXDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (UnnamedObjMVIDXDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
