    ...
    public ArrayList<UnnamedObjSVDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<UnnamedObjSVDMO> rc = new  ArrayList<UnnamedObjSVDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(UnnamedObjSVDMO val: value)
                        rc.add((UnnamedObjSVDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (UnnamedObjSVDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
