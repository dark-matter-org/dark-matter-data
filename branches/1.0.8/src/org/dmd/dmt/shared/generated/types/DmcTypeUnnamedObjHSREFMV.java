    ...
    public ArrayList<UnnamedObjHSDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<UnnamedObjHSDMO> rc = new  ArrayList<UnnamedObjHSDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(UnnamedObjHSDMO val: value)
                        rc.add((UnnamedObjHSDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (UnnamedObjHSDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
