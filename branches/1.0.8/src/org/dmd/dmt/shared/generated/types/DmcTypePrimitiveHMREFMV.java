...
public class DmcTypePrimitiveHMREFMV extends DmcTypePrimitiveHMREF implements Serializable {
    ...
    public DmcTypePrimitiveHMREFMV getNew(){
        return(new DmcTypePrimitiveHMREFMV(attrInfo));
    }
    ...
    public ArrayList<PrimitiveHMDMO> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<PrimitiveHMDMO> rc = new  ArrayList<PrimitiveHMDMO>(value.size());
                if (attrInfo.indexSize == 0){
                    for(PrimitiveHMDMO val: value)
                        rc.add((PrimitiveHMDMO) val.cloneIt());
                }
                else{
                    ...
                    for(int index=0; index<value.size(); index++){
                        if (value.get(index) != null)
                            rc.set(index, (PrimitiveHMDMO) value.get(index).cloneIt());
                    }
                }
                return(rc);
            }
    ...
    public PrimitiveHMDMO setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<PrimitiveHMDMO>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
