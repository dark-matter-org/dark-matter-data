...
/**
 * The DmcTypeWebApplicationREFMV provides storage for a multi-valued WebApplicationREF
 * <P>
...
public class DmcTypeWebApplicationREFMV extends DmcTypeWebApplicationREF implements Serializable {
    ...
    public DmcTypeWebApplicationREFMV(DmcAttributeInfo ai){
        super(ai);
    }
    
    @Override
    public DmcTypeWebApplicationREFMV getNew(){
        return(new DmcTypeWebApplicationREFMV(attrInfo));
    }
    ...
    public WebApplicationREF add(Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null)
                value = new ArrayList<WebApplicationREF>();
            value.add(rc);
            return(rc);
        }
    }
    ...
    public ArrayList<WebApplicationREF> getMVCopy(){
        synchronized(this){
            if (value == null)
                ...
                ArrayList<WebApplicationREF> rc = new  ArrayList<WebApplicationREF>(value.size());
                if (attrInfo.indexSize == 0){
                    for(WebApplicationREF val: value)
                        rc.add(val);
                }
    ...
    public WebApplicationREF setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<WebApplicationREF>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
