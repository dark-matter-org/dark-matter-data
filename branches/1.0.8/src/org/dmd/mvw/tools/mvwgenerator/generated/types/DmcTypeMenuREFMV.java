...
public class DmcTypeMenuREFMV extends DmcTypeMenuREF implements Serializable {
    ...
    private final static Iterator<MenuREF> emptyList = (new ArrayList<MenuREF>()).iterator();
    
    protected ArrayList<MenuREF> value;
    
    public DmcTypeMenuREFMV(){
    
    }
    
    public DmcTypeMenuREFMV(DmcAttributeInfo ai){
        super(ai);
    }
    
    @Override
    public DmcTypeMenuREFMV getNew(){
        return(new DmcTypeMenuREFMV(attrInfo));
    }
    ...
    public ArrayList<MenuREF> getMVCopy(){
        synchronized(this){
            if (value == null)
                return(new ArrayList<MenuREF>());
            else{
                ArrayList<MenuREF> rc = new  ArrayList<MenuREF>(value.size());
                if (attrInfo.indexSize == 0){
                    for(MenuREF val: value)
                        rc.add(val);
                }
    ...
    public MenuREF setMVnth(int index, Object v) throws DmcValueException {
        synchronized(this){
            ...
            if (value == null){
                value = new ArrayList<MenuREF>(attrInfo.indexSize);
                for(int i=0;i<attrInfo.indexSize;i++)
                    value.add(null);
            }
