...
public class DmcTypeSetResponseREFMAP extends DmcTypeSetResponseREF implements Serializable {
    ...
    public SetResponseREF add(Object v) throws DmcValueException {
        SetResponseREF newval = typeCheck(v);
        if (value == null)
            initValue();
        IntegerName key = (IntegerName)((DmcMappedAttributeIF)newval).getKey();
        SetResponseREF oldval = value.put(key,newval);
    ...
    public Iterator<SetResponseREF> getMV(){
        Map<IntegerName,SetResponseREF> clone = null;
        if (attrInfo.valueType == ValueTypeEnum.HASHMAPPED)
            clone = new HashMap<IntegerName,SetResponseREF>(value);
        else
            clone = new TreeMap<IntegerName,SetResponseREF>(value);
        return(clone.values().iterator());
    }
    
    public Map<IntegerName,SetResponseREF> getMVCopy(){
        Map<IntegerName,SetResponseREF> clone = null;
        if (attrInfo.valueType == ValueTypeEnum.HASHMAPPED)
            clone = new HashMap<IntegerName,SetResponseREF>(value);
        else
